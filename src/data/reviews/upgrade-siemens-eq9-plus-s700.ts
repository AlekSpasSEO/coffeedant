...
const officialSpec = 'https://media3.bsh-group.com/Documents/specsheet/en-GB/TI9573X9GB.pdf';
...
const profile: UpgradeReviewProfile = {
  ...
  facts: [
    ...
    { label: 'Bean storage', value: 'dualBean System with two bean containers, each with its own grinder; official overview lists 250 g capacity' },
    ...
    { label: 'Controls', value: 'Colour display, rotary control, up to 10 profiles and baristaMode recipe adjustment' },
  ...
  quickAnswers: [
    ...
    { question: 'Can it keep regular and decaf beans ready?', answer: 'Yes. Siemens documents two bean containers, each with its own grinder, and lets the user choose a bean side for recipes. Verify both paths on a used unit and expect some retained grounds rather than assuming medical-grade separation.' },
  ...
  workflowNote: 'The dualBean System is the daily reason to choose this older flagship. Siemens documents two bean containers, each with its own grinder, so a household can keep regular and decaf or two roast styles ready, select the appropriate side and save repeat recipes in up to 10 profiles. The large tank, paired drinks, cup heater and Home Connect reduce repeated steps. Automatic rinses consume water and fill the tray, retained grounds can still remain in a coffee path, and connected commands require a ready cup and clean milk path. The 470 mm depth also needs space behind and above the machine for ventilation, lids and service access.',
  buildNote: 'Siemens lists a 392 × 299 × 470 mm cabinet, 2.3 L water tank, dualBean System with a separate grinder for each bean container, switchable cup heater and integrated milk hose. Amazon UK lists roughly 12.5 kg and 1,500 W for the exact model, but retailer values remain secondary to the rating plate and official documents. The official TI9573X9GB/21 service page still provides manuals, product documents, repair booking and original spare-part routes. That is useful infrastructure, not a promise that every display, board, valve, grinder or seal remains economical or immediately stocked.',
  featuresNote: 'The useful feature set is specific rather than decorative: two bean containers with separate grinders, baristaMode, up to 10 profiles, aromaDouble Shot, oneTouch DoubleCup, autoMilk Clean, ceramDrive, cup heating and Home Connect. Some functions depend on firmware, account and app region. The page does not convert the official 4.6/5 rating from 25 reviews or Amazon’s 3.8/5 from 19 ratings into a reliability rate, because neither sample is controlled and ownership duration is unknown.',
  ...
  sources: [
    ...
    { label: 'Siemens UK TI9573X9GB specification sheet', href: officialSpec, note: 'Exact-model official PDF confirming the dualBean System with a separate grinder for each bean container, baristaMode and up to 10 individual profiles.' },
  ],
  comparisons: [
  ...
  faqs: [
    ...
    { question: 'Does it have two grinders?', answer: 'Yes. Siemens’ exact TI9573X9GB specification sheet states that the two bean containers each have their own grinder. A used buyer should still test both bean paths because the document cannot establish the condition of an individual machine.' },
...
profile.sectionOverrides = {
  ...
  workflow: `<p>${profile.workflowNote} <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-1">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-6">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-11">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-13">[source]</a></p>`,
  build: `<p>${profile.buildNote} <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-1">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-2">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-7">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-13">[source]</a></p><p>${profile.maintenanceNote}</p>`,
  features: `<p>${profile.featuresNote} <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-1">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-7">[source]</a> <a href="#siemens-eq9-plus-s700-ti9573x9gb-uk-source-13">[source]</a></p>`,
